...
// 检查助手回复是否为空（仅含空白字符）
// Checks whether an assistant response is empty (whitespace only)
function isEmptyAssistantResponse(content: string): boolean {
  return content.trim().length === 0
}

// 将 providerUsage 附加到符合条件的助手/工具调用消息上
// Attaches providerUsage metadata to eligible assistant/tool-call messages
...
// 判断助手消息是否应该视为进度更新而非最终回复
// Determines whether an assistant message should be treated as a progress update rather than a final answer
...
// 格式化 stop_reason、blockTypes 等诊断信息为中文提示字符串
// Formats diagnostics (stop_reason, blockTypes, etc.) into a Chinese-language hint string
...
// 检查因 thinking 阶段被截断（pause_turn / max_tokens）导致的空响应是否可重试恢复
// Checks whether an empty response caused by a truncated thinking phase (pause_turn / max_tokens) is recoverable via retry
...
// 执行一轮完整 agent 循环：上下文压缩、调用模型、执行工具调用，直到模型返回最终回复或达到最大步数
// Executes a full agent turn: context compression, model invocation, tool execution, looping until the model returns a final answer or max steps are reached
