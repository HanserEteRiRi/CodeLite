import { z } from 'zod'
import type { ToolDefinition } from '../tool.js'
import { loadSkill } from '../skills.js'

type Input = {
  name: string
}

// 创建用于按名称加载技能文件完整内容的工具定义
// Create a tool definition for loading the full content of a skill file by name
export function createLoadSkillTool(cwd: string): ToolDefinition<Input> {
  return {
    name: 'load_skill',
    description:
      'Load the full contents of a named SKILL.md file so you can follow that workflow accurately.',
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string' },
      },
      required: ['name'],
    },
    schema: z.object({
      name: z.string().min(1),
    }),
    // 加载并返回指定技能的内容，未找到时返回错误信息
    // Load and return a named skill's content, or an error if not found
    async run(input) {
      const skill = await loadSkill(cwd, input.name)
      if (!skill) {
        return {
          ok: false,
          output: `Unknown skill: ${input.name}`,
        }
      }

      return {
        ok: true,
        output: [
          `SKILL: ${skill.name}`,
          `SOURCE: ${skill.source}`,
          `PATH: ${skill.path}`,
          '',
          skill.content,
        ].join('\n'),
      }
    },
  }
}
