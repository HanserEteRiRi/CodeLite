...
// 构建未执行剪裁的结果对象（包含跳过原因）
// Build a result object indicating no snipping was performed (with skip reason)
...
// 判断消息是否属于不可剪裁的边界（system、context_summary、snip_boundary）
// Check whether a message is a non-snippable boundary (system, context_summary, snip_boundary)
...
// 判断工具名是否属于受保护的文件编辑类工具（不可剪裁）
// Check whether a tool name belongs to protected file-editing tools (non-snippable)
...
// 判断工具结果是否包含重要错误（isError 或内容含错误关键词）
// Check whether a tool result contains an important error (isError or content has error keywords)
...
// 判断用户/助手类消息的文本是否包含重要错误关键词
// Check whether user/assistant message text contains important error keywords
...
// 判断消息组是否包含受保护的文件编辑工具调用或结果
// Check whether a message group contains protected file-editing tool calls or results
...
// 判断消息组是否包含重要错误信息
// Check whether a message group contains important error information
...
// 将消息数组构建为消息组：配对 tool_call+tool_result，孤立项标记为受保护
// Build message groups from the array: pair tool_call+tool_result, orphan items marked as protected
...
// 将消息组标记为受保护并记录保护原因
// Mark a message group as protected and record the protection reason
...
// 将指定索引附近的消息组（前后各一个）标记为受保护
// Mark the message groups near the given index (one before and one after) as protected
...
// 标记候选范围内的受保护组：边界消息、文件编辑附近、错误附近的组均不可剪裁
// Mark protected groups in the candidate range: boundaries, near-file-edits, near-errors are all non-snippable
...
// 查找可剪裁的候选范围：跳过保留的近期消息和边界消息后的中间区域
// Find the snippable candidate range: the middle region after skipping recent kept messages and boundaries
...
// 在被保护组之间找到连续的非保护组序列（安全可剪裁区间）
// Find contiguous non-protected group sequences between protected groups (safe snippable intervals)
...
// 比较两个安全区间：优先按 token 数降序，其次按消息数降序，最后按起始位置升序
// Compare two safe runs: first by tokens descending, then message count descending, then start position
...
// 从安全区间中选择需要删除的子范围：累积足够 token 且满足最小消息数要求
// Select a deletion sub-range from a safe run: accumulate enough tokens meeting min message count
...
// 构建剪裁边界消息的文本内容，说明移除了多少消息和 token
// Build the snip boundary message text describing how many messages and tokens were removed
...
// 构建适用于 Anthropic API 的简洁剪裁边界文本（不含 token 统计信息）
// Build a concise snip boundary text for the Anthropic API (without token statistics)
...
// 构建 snip_boundary 角色消息，标记剪裁位置并携带元数据
// Build a snip_boundary role message marking the snip position with metadata
...
// 将保留的消息中的 provider usage 标记为过期（因为上下文已被剪裁修改）
// Mark provider usage as stale on retained messages (since context was modified by snipping)
...
// 剪裁压缩主入口：确定性地移除对话中间的安全区间以释放 token，不调用 LLM
// Snip-compact main entry: deterministically remove a safe middle interval to free tokens, no LLM call
