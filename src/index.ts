...
// CodeLite 入口：解析 CLI 参数、加载配置、创建模型适配器，然后启动交互模式或行读取循环
// Entry point: parses CLI arguments, loads configuration, creates a model adapter, then starts interactive mode or a readline loop
async function main(): Promise<void> {
  ...
  const model =
    process.env.CODE_LITE_MODEL_MODE === 'mock'
  ...
  // 重新构建系统提示词，刷新 skills、MCP 服务器等最新信息
  // Rebuilds the system prompt with the latest skills, MCP servers, and other metadata
