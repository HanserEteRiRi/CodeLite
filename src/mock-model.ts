...
// 从消息历史中提取最后一条用户消息的内容
// Extracts the content of the last user message from the message history
...
// 从消息历史中提取最后一条工具结果消息
// Extracts the last tool_result message from the message history
...
// 从消息历史中提取最近一次助手工具调用的工具名称
// Extracts the tool name of the most recent assistant tool call from the message history
...
export class MockModelAdapter implements ModelAdapter {
  // 模拟模型调用：根据用户输入和工具结果，返回预设的 mock 回复或工具调用
  // Simulates a model call: returns pre-defined mock responses or tool calls based on user input and tool results
