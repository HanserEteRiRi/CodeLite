...
import { CODE_LITE_PROJECTS_DIR } from '../src/config.js'
...
const testDir = path.join(os.tmpdir(), 'codelite-session-test')
...
async function cleanupAll() {
  try { await rm(CODE_LITE_PROJECTS_DIR, { recursive: true, force: true }) } catch {}
}
...
describe('session persistence', () => {
  ...
  it('stores events in envelope jsonl format', async () => {
    ...
    const pdir = path.join(CODE_LITE_PROJECTS_DIR, projectDirName(cwd))
